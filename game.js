...
const floorTiles = FLOOR_TILE_FILES.map(src =>
  loadImageCandidates([src, `assets/floor/${src}`])
);
...
const propImages = Object.fromEntries(
  Object.entries(PROP_FILES).map(([key, src]) => [key, loadImageCandidates([src])])
);

const WALL_FILES = {
  top: [
    ['walls/top/top-wall-clean-1.png', 'top-wall-clean.png'],
    ['walls/top/top-wall-clean-2.png', 'top-wall-clean.png'],
    ['walls/top/top-wall-clean-3.png', 'top-wall-clean.png'],
    ['walls/top/top-wall-cracked-1.png', 'top-wall-cracked.png'],
    ['walls/top/top-wall-cracked-2.png', 'top-wall-cracked.png'],
    ['walls/top/top-wall-cracked-3.png', 'top-wall-cracked.png'],
    ['walls/top/top-wall-mossy-1.png', 'top-wall-mossy.png'],
    ['walls/top/top-wall-mossy-2.png', 'top-wall-mossy.png'],
    ['walls/top/top-wall-mossy-3.png', 'top-wall-mossy.png']
  ],
...
const wallImages = {
  top: WALL_FILES.top.map(loadImageCandidates),
  left: loadImageCandidates(WALL_FILES.left[0]),
  right: loadImageCandidates(WALL_FILES.right[0]),
  bottom: loadImageCandidates(WALL_FILES.bottom[0])
};
...
function roomSeed(offset = 0) {
  return area * 1009 + room * 131 + roomVariation * 17 + offset;
}

function makeRoomLayout() {
  roomTiles = Array.from({ length: ROWS }, (_, y) =>
    Array.from({ length: COLS }, (_, x) => {
      const roll = seededRandom(roomSeed(y * COLS + x));
      let family = 0;
      ...
      if (family === 2 && edgeDistance > 2 && roll > 0.55) family = 0;
      if (family === 1 && edgeDistance === 0 && roll < 0.35) family = 2;

      return family * 4 + Math.floor(seededRandom(roomSeed(100 + y * COLS + x)) * 4);
    })
  );

  // Keep the central combat lane readable.
  const centreColumn = Math.floor(COLS / 2);
  for (let y = 2; y < ROWS - 2; y++) {
    if (roomTiles[y][centreColumn] >= 4 && roomTiles[y][centreColumn] < 8) {
      roomTiles[y][centreColumn] = Math.floor(seededRandom(roomSeed(300 + y)) * 4);
    }
  }

  const placed = [];
  const addDecor = (kind, x, y, torch = false) => {
    if (x < ROOM_X + 16 || x > ROOM_X + ROOM_W - 16) return;
    if (y < ROOM_Y + 18 || y > ROOM_Y + ROOM_H - 18) return;
    if (!torch && y > ROOM_Y + ROOM_H - 55) return;
    if (!torch && Math.abs(x - W / 2) < 55 && y > ROOM_Y + 55) return;
    if (placed.some(item => Math.hypot(item.x - x, item.y - y) < 38)) return;
    placed.push({ kind, x, y, torch });
  };

  const torchSlots = [
    { x: 72, y: 88 }, { x: 288, y: 88 },
    { x: 72, y: 122 }, { x: 288, y: 122 }
  ];
  const torchCount = 2 + Math.floor(seededRandom(roomSeed(401)) * 3);
  for (let i = 0; i < torchCount; i++) {
    const slot = torchSlots[(i + Math.floor(seededRandom(roomSeed(410 + i)) * torchSlots.length)) % torchSlots.length];
    const kind = `torch${1 + Math.floor(seededRandom(roomSeed(420 + i)) * 4)}`;
    addDecor(kind, slot.x, slot.y, true);
  }
  ...
  const shuffled = candidates
    .map((slot, index) => ({ slot, order: seededRandom(roomSeed(500 + index)) }))
    .sort((a, b) => a.order - b.order)
    .map(item => item.slot);

  const decorCount = 3 + Math.floor(seededRandom(roomSeed(600)) * 3);
  for (let i = 0, added = 0; i < shuffled.length && added < decorCount; i++) {
    const slot = shuffled[i];
    const kind = DECORATION_POOL[Math.floor(seededRandom(roomSeed(610 + i)) * DECORATION_POOL.length)];
    const before = placed.length;
    addDecor(kind, slot.x, slot.y);
    if (placed.length > before) added++;
  }
...
function drawFloor() {
  ...
  for (let y = 0; y < ROWS; y++) {
    for (let x = 0; x < COLS; x++) {
      ...
      if (tile?.ready) {
        ctx.drawImage(tile, ROOM_X + x * TILE, ROOM_Y + y * TILE, TILE, TILE);
      }
    }
  }
  ctx.restore();
...
function topWallIndex() {
  const archetype = roomArchetype();
  const roll = seededRandom(roomSeed(700));
  let family = 0;

  if (archetype === 'RUINED CHAMBER') {
    family = roll < 0.42 ? 1 : roll < 0.72 ? 2 : 0;
  } else {
    family = roll < 0.10 ? 2 : roll < 0.30 ? 1 : 0;
  }

  return family * 3 + (roomVariation + room * 3 + area) % 3;
}

function drawWalls() {
  const drawImage = (image, x, y, width, height, sourceY = 0, sourceHeight = null) => {
    if (!image?.ready) return;
    const sourceWidth = image.naturalWidth || image.width;
    const fullHeight = image.naturalHeight || image.height;
    const cropHeight = sourceHeight ?? fullHeight;
    ctx.drawImage(image, 0, sourceY, sourceWidth, cropHeight, x, y, width, height);
  };

  const topIndex = topWallIndex();
  const top = wallImages.top[topIndex];
  const topIsPackedFallback = !top?.src.includes('walls/top/');
  drawImage(
    top,
    0,
    16,
    W,
    50,
    topIsPackedFallback && topIndex >= 3 ? (topIndex % 3) * 50 : 0,
    topIsPackedFallback && topIndex >= 3 ? 50 : null
  );

  drawImage(wallImages.left, 0, 16, 52, H - 66);
  drawImage(wallImages.right, W - 50, 16, 50, H - 66);
  drawImage(wallImages.bottom, 0, H - 50, W, 50);
}

function drawRoomDecor() {
  ctx.imageSmoothingEnabled = false;
  for (const decor of roomDecor) {
    const image = propImages[decor.kind];
    if (!image?.ready) continue;
    ctx.drawImage(image, Math.round(decor.x - 16), Math.round(decor.y - 16), 32, 32);
  }
}
...
function drawDungeon() {
  ...
  for (const torch of roomDecor.filter(item => item.torch)) {
    const pulse = 0.16 + 0.04 * Math.sin(time * 4 + torch.x);
    const glow = ctx.createRadialGradient(torch.x, torch.y - 10, 2, torch.x, torch.y - 10, 42);
    glow.addColorStop(0, `rgba(255,154,58,${pulse})`);
    glow.addColorStop(1, 'rgba(0,0,0,0)');
    ctx.fillStyle = glow;
    ctx.fillRect(torch.x - 44, torch.y - 54, 88, 88);
  }
}
